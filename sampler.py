...
class Sampler():
    ...
    def initNoise(self, frequency=0.02, octaves=6, lacunarity=2, gain=0.45,
        ...
        noise = fns.Noise(seed=seed, numWorkers=numWorkers)

        noise.noiseType = fns.NoiseType.PerlinFractal
        noise.fractal.fractalType = fns.FractalType.FBM
    ...
    def generate_ndNoise(self, noiseObj, xv, yv, zv):
        ...
        return noiseObj.genFromCoords(coords).reshape(len(xv), len(yv), len(zv))
    ...
    def save_music_anim_gif(self, z1, r_global, audiopath, filename, n_frame=10, start_frame=0,
                            x_dim=512, y_dim=512, scale=10.0,
                            f_params=None, img=None, acceleration=False, exp_gain=1,
                            fps=30, w_scaler=2, normalize_w = True, f_amp_w=False):
        '''
        this saves a png sequence (no longer gif) by rotating the latent vector z1 360 degres back to itself while
        being driven by musical amplitude changes.
        r: radius of rotation
        n_frame: number of states in between z1 and z2 morphing effect, exclusive of z1 and z2
        duration1, duration2, control how long z1 and z2 are shown.  duration controls frame speed, in seconds
        ...
        delta_z1 = self.calculate_delta_z(z1, r_global, audiopath, n_frame, start_frame,
                                    acceleration, exp_gain, fps, w_scaler, normalize_w, f_amp_w)

        delta_z2 = self.calculate_delta_z(z1, 1, audiopath, n_frame, start_frame,
                                             acceleration=False, exp_gain=0.5, fps=fps, w_scaler=w_scaler,
                                             normalize_w=False,
                                             f_amp_w=False)
        ...
        for i in range(total_frames):
            ...
            image = (255 * self.generate(z, x_dim, y_dim, scale,
                                               f_params=f_params, img=img_in)).astype(np.uint8)
    ...
    def calculate_delta_z(self, z1, r_global, audiopath, n_frame=10, start_frame=0,
        ...
        amp = self.generate_amp(sound, fs, fps)[start_frame:start_frame + total_frames]
        start_frame = 0
        ...
        if f_amp_w:
            ...
            cumamp = -np.cumsum(amp, axis=0)
            cumamp = np.divide(cumamp, np.max(np.abs(cumamp), axis=0))
            cumamp = np.sin(cumamp * freq * np.pi)

            delta_z1 = r_amp * cumamp * np.sin(theta1)
            delta_z2 = r_amp * cumamp * np.cos(theta2)
...
#                             n_frame=322, start_frame=start_frame, img=None,
